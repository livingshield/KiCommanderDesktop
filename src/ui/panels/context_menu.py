    ...
    def build_and_show(self, pos):
        ...
        if index.isValid():
            ...
            if file_info and file_info.name != "..":
                ...
                # Basic Ops
                menu.addAction(qta.icon("fa5s.eye", color="#a6e3a1"), "View (F3)", lambda: bus.action_requested.emit("view"))
                menu.addAction(qta.icon("fa5s.edit", color="#f9e2af"), "Edit (F4)", lambda: bus.action_requested.emit("edit"))
                ...
                # File Operations
                menu.addAction(qta.icon("fa5s.copy", color="#89b4fa"), "Copy (F5)", lambda: bus.action_requested.emit("copy"))
                menu.addAction(qta.icon("fa5s.external-link-alt", color="#fab387"), "Move (F6)", lambda: bus.action_requested.emit("move"))
                menu.addAction(qta.icon("fa5s.file-archive", color="#cba6f7"), "Pack files (Alt+F5)", lambda: bus.action_requested.emit("archive"))
                
                menu.addSeparator()
                
                # Clipboard
                menu.addAction(qta.icon("fa5s.cut", color="#cdd6f4"), "Cut (Ctrl+X)", lambda: bus.action_requested.emit("clipboard_cut"))
                menu.addAction(qta.icon("fa5s.clone", color="#cdd6f4"), "Copy (Ctrl+C)", lambda: bus.action_requested.emit("clipboard_copy"))
                menu.addAction(qta.icon("fa5s.paste", color="#cdd6f4"), "Paste (Ctrl+V)", lambda: bus.action_requested.emit("clipboard_paste"))
                
                menu.addSeparator()
                
                # Management
                menu.addAction(qta.icon("fa5s.i-cursor", color="#f9e2af"), "Rename (F2)", lambda: self.p.rename_file(file_info))
                menu.addAction(qta.icon("fa5s.trash-alt", color="#f38ba8"), "Delete (F8)", lambda: bus.action_requested.emit("delete"))
                ...
                menu.addAction(qta.icon("fa5s.folder-plus", color="#a6e3a1"), "New Folder (F7)", lambda: bus.action_requested.emit("mkdir"))
                menu.addAction(qta.icon("fa5s.paste", color="#cdd6f4"), "Paste (Ctrl+V)", lambda: bus.action_requested.emit("clipboard_paste"))
        else:
            # Click on empty area
            menu.addAction(qta.icon("fa5s.folder-plus", color="#a6e3a1"), "New Folder (F7)", lambda: bus.action_requested.emit("mkdir"))
